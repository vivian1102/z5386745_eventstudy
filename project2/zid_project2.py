...
def read_prc_csv(tic):
    ...
    csv_file_path = os.path.join(cfg.DATADIR, f"{tic}_prc.csv")
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    for ticker in tickers:
        df_data = read_prc_csv(ticker)
        df_prc = df_data.loc[:,[prc_col]]
        df_prc.columns = [f"{ticker.lower()}"]
        df = df.join(df_prc, how="outer")
