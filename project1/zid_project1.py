...
def get_tics(pth):
    ...
    with open(pth, mode= 'r') as fobj:
        ...
        for e in fobj:
            e = e.strip()
            exchange, tic = tuple(e.replace('\"', '').lower().split('='))
            ticinf[tic] = exchange
        return ticinf
...
def line_to_dict(line):
    ...
    i = 0
    for key in COLUMNS:
        tickers[key] = line[i:i+COLWIDTHS[key]]
        i += COLWIDTHS[key]
    return tickers
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        dict1 = {}
        dict1['exchange'] = tic_exchange_dic[tic]

        data = []
        lines = read_dat(tic)
        for line in lines:
            inf = {col: line_to_dict(line)[col] for col in col_lst}
            data.append(inf)
